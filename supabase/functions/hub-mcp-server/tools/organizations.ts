...
export const handlers: ToolModule["handlers"] = {
  async create_organization(args, { admin, userId }) {
    ...
    // Hub-wide switch (superadmin Settings page) — false means only a platform admin can create
    // orgs (via admin_create_organization), everyone else must be invited into an existing one.
    const { data: setting, error: settingErr } = await admin.from("hub_platform_settings").select("value").eq("key", "org_self_signup_enabled").maybeSingle();
    if (settingErr) throw new Error(settingErr.message);
    if (setting && setting.value === false) throw new Error("Self-serve organization creation is disabled. Ask a platform admin to create one for you.");

    const { data: org, error } = await admin.from("hub_organizations").insert({ name }).select("id, name, created_at").single();
    if (error) throw new Error(error.message);
    const { error: memErr } = await admin.from("hub_organization_members").insert({ user_id: userId, organization_id: org.id, role: "owner" });
  ...
  async list_my_organizations(_args, { admin, userId }) {
    ...
    const { data, error } = await admin.from("hub_organization_members").select("role, hub_organizations(id, name, created_at)").eq("user_id", userId);
    if (error) throw new Error(error.message);
    return (data ?? []).map((r: { role: string; hub_organizations: unknown }) => ({ ...(r.hub_organizations as object), role: r.role }));
  },
  ...
  async create_project(args, { admin, userId }) {
    ...
    const { data: membership, error: memLookupErr } = await admin
      .from("hub_organization_members").select("role").eq("organization_id", organizationId).eq("user_id", userId).maybeSingle();
    if (memLookupErr) throw new Error(memLookupErr.message);
    if (!membership) throw new Error("Not a member of this organization.");

    const { data: project, error } = await admin.from("hub_projects").insert({ organization_id: organizationId, name }).select("id, organization_id, name, created_at").single();
    if (error) throw new Error(error.message);
    const { error: pmErr } = await admin.from("hub_project_members").insert({ user_id: userId, project_id: project.id, role: "owner" });
  ...
  async list_my_projects(args, { admin, userId }) {
    ...
    let q = admin.from("hub_project_members").select("role, hub_projects(id, organization_id, name, created_at)").eq("user_id", userId);
    const { data, error } = await q;
    if (error) throw new Error(error.message);
    let rows = (data ?? []).map((r: { role: string; hub_projects: unknown }) => ({ ...(r.hub_projects as { organization_id: string }), role: r.role }));
