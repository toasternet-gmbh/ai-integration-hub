...
export const handlers: ToolModule["handlers"] = {
  async create_agent(args, { admin, projectId }) {
    ...
    const { data, error } = await admin
      .from("hub_agents").insert({ project_id: projectId, name, description: args.description ? String(args.description) : null })
  ...
  async list_agents(_args, { admin, projectId }) {
    const { data, error } = await admin.from("hub_agents").select("id, name, description, status, created_at").eq("project_id", projectId).order("created_at", { ascending: false });
    if (error) throw new Error(error.message);
    return data;
  },
  ...
  async set_agent_tool_permission(args, { admin, projectId }) {
    ...
    if (existing) {
      const { data, error } = await admin.from("hub_agent_tool_permissions").update({ permission }).eq("id", existing.id).select("id, tool_name, integration_id, permission").single();
      if (error) throw new Error(error.message);
      return data;
    }
    const { data, error } = await admin
