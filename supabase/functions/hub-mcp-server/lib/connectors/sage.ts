/**
 * Stub connector for Sage (Sage Business Cloud Accounting / Sage Intacct).
...
export class SageConnector implements Connector {
  ...
  async execute(tool: string, _input: Record<string, unknown>): Promise<ToolResult> {
    throw new Error(`Sage: ${tool} is implemented as a stub and cannot execute real calls yet.`);
  }
}
