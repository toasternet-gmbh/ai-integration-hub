 ...
 * auth is itself dynamic — an `authProvider` query returns a per-account OpenID Connect
 * configuration (`clientIdentification`, `clientSecret`, `authorizeEndpoint`, `tokenEndpoint`),
 * meaning the actual OAuth endpoints aren't a fixed, publicly documented URL the way even DATEV's
 * are, but discovered per tenant. Confirming the exact discovery query and the projects/workdays
 * query/mutation shapes needs a live account or a direct line to 123erfasst (their own public
 * docs describe "importing master data" and "exporting all data" only in prose, not schema) —
 * neither was available this session.
 *
 * Ships with `hub_platform_types.enabled = false` and is expected to stay that way until a real
 * account's `authProvider` response and GraphQL schema can be inspected directly. Do not treat
 * this file as verified.
 */
