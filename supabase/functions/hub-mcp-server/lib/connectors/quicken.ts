...
export class QuickenConnector implements Connector {
  ...
  async execute(tool: string, _input: Record<string, unknown>): Promise<ToolResult> {
    throw new Error(`Quicken: ${tool} is implemented as a stub and cannot execute real calls yet.`);
  }
}
