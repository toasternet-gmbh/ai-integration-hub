/**
 * Auth for the MCP gateway (API key / JWT), no OAuth-discovery dance — a Hub client authenticates
 * with either a project-scoped API key, or a human's own Supabase JWT plus an explicit
 * project_id param.
 */
...
export async function authenticateMcpRequest(
  ...
  if (token.startsWith("hub_")) {
    ...
    const { data, error } = await admin.from("hub_api_keys").select("id, project_id").eq("key_hash", hash).maybeSingle();
    if (error) throw new McpAuthError(error.message);
    if (!data) throw new McpAuthError("Invalid API key");
    admin.from("hub_api_keys").update({ last_used_at: new Date().toISOString() }).eq("id", data.id).then(() => {});
  ...
  const { data: membership, error: memErr } = await admin
    .from("hub_project_members").select("project_id").eq("project_id", requestedProjectId).eq("user_id", user.id).maybeSingle();
