...
const STATUS_LABEL: Record<string, string> = { allowed: "Success", executed: "Success", require_approval: "Pending approval", denied: "Denied", error: "Error" };
  ...
  return (
    <div className="flex flex-col w-full p-margin-page gap-margin-page max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-gutter">
        ...
        <StatCard label="System Errors" icon="check_circle" value={errorsToday} suffix={t("dashboard.stat.errors")} tone={errorsToday > 0 ? "danger" : "ok"} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter">
        <div className="lg:col-span-8 flex flex-col bg-surface-container rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="flex flex-col w-full">
            <div className="grid grid-cols-12 gap-4 px-gutter py-2 border-b border-outline-variant bg-surface-container-lowest">
              <div className="col-span-4 font-label-caps text-label-caps text-on-surface-variant uppercase">Timestamp</div>
              <div className="col-span-5 font-label-caps text-label-caps text-on-surface-variant uppercase">Tool</div>
            ...
            {logs.map((r) => (
              <div key={r.id} className="grid grid-cols-12 gap-4 px-gutter py-3 items-center border-b border-outline-variant bg-surface last:border-b-0">
                ...
                <div className="col-span-3 flex items-center gap-2">
                  ...
                  <span className="font-body-md text-[13px] text-on-surface-variant">{STATUS_LABEL[r.status] ?? r.status}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-component-gap">
          <div className="flex items-center justify-between pb-2 border-b border-outline-variant">
            ...
            {approvals.length > 0 && (
              <div className="bg-[#b5730b] text-white font-label-caps text-[10px] px-2 py-0.5 rounded-full">{approvals.length} PENDING</div>
            )}
          </div>
          {approvals.length === 0 && <p className="font-body-md text-body-md text-on-surface-variant">{t("dashboard.nothingPending")}</p>}
          {approvals.map((a) => (
            <div key={a.id} className="bg-surface-container rounded-xl p-gutter flex flex-col gap-4 border border-outline-variant shadow-sm">
              ...
              <div className="flex gap-3 pt-2 border-t border-outline-variant">
                <button onClick={() => decide(a.id, "approve")} className="flex-1 bg-primary text-on-primary font-body-md text-[14px] font-medium py-2 px-4 rounded hover:bg-primary-container transition-colors flex justify-center items-center gap-2">
...
function StatCard({ label, icon, value, suffix, tone = "neutral" }: { label: string; icon: string; value: number; suffix: string; tone?: "neutral" | "warn" | "ok" | "danger" }) {
  const toneClasses = {
    ...
    warn: "bg-[#fff8e1] text-[#b5730b] border border-[#b5730b]/20",
    ok: "bg-[#e6f4ea] text-[#1f8a5a] border border-[#1f8a5a]/20",
    danger: "bg-error-container text-on-error-container border border-error/20",
  }[tone];
  const numberColor = tone === "warn" ? "text-[#b5730b]" : tone === "ok" ? "text-[#1f8a5a]" : tone === "danger" ? "text-on-error-container" : "text-on-surface";
  return (
    <div className={`flex flex-col rounded-xl p-gutter relative overflow-hidden ${toneClasses}`}>
