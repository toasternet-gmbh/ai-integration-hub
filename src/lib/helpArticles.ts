  ...
  },
  {
    ...
    body: {
      en: [
        ...
        "The Quick Start panel on that page shows a minimal working call using the official JS/TS client, `@ai-integration/hub` — copy it as a starting point for your own integration. It's not on the public npm registry yet; install it with `npm install github:toasternet-gmbh/hub-sdk` instead (a dedicated single-package repo, so plain npm can install it directly — it builds itself on install). Full usage docs, including the `require_approval` handling pattern, are in that package's own README.",
      ],
      de: [
        ...
        "Das Schnellstart-Panel auf dieser Seite zeigt einen minimalen funktionierenden Aufruf mit dem offiziellen JS/TS-Client `@ai-integration/hub` — als Ausgangspunkt für Ihre eigene Integration kopierbar. Er ist noch nicht im öffentlichen npm-Registry; installieren Sie ihn stattdessen mit `npm install github:toasternet-gmbh/hub-sdk` (ein eigenes Repo nur für dieses Paket, sodass ein normales npm-Install direkt funktioniert — der Build läuft automatisch beim Installieren). Die vollständige Nutzungsdokumentation, einschließlich des Umgangs mit `require_approval`, steht im README dieses Pakets.",
      ],
    },
  },
  {
    ...
    category: "developer",
    readMins: 4,
    title: { en: "Using the @ai-integration/hub SDK", de: "Der @ai-integration/hub-SDK im Einsatz" },
    summary: {
      en: "Install, make your first call, and handle require_approval and errors with the official client.",
      de: "Installation, der erste Aufruf, und der Umgang mit require_approval und Fehlern im offiziellen Client.",
    },
    body: {
      en: [
        "`@ai-integration/hub` wraps the raw JSON-RPC calls behind `client.call(name, input)` and a typed, dot-path shortcut — `client.tools.orders.refund({...})`, `client.tools.cms.pages.search({...})` — for every canonical tool. It authenticates with a project API key, so it's built for a server-side agent or script, not a page a customer's browser loads directly.",
        "## Install",
        "Not on the public npm registry yet — install it from its dedicated repo instead: `npm install github:toasternet-gmbh/hub-sdk`. It builds itself automatically on install, so there's no separate clone-and-build step. Pin a version instead of tracking the default branch with `npm install github:toasternet-gmbh/hub-sdk#v0.1.0`.",
        "## Make your first call",
        "Construct one client per process: `new HubClient({ baseUrl, apiKey, agentId })`. `baseUrl` is your Hub's MCP gateway URL (ends in `/functions/v1/hub-mcp-server`), `apiKey` comes from the API Keys page, and `agentId` names which agent this process acts as — most tools require it, though it can be overridden per call instead of set once. Then call any tool by its dot-path, e.g. `client.tools.orders.search({ integration_id, status: 'pending_approval', limit: 10 })`.",
        "## Handling require_approval",
        "A call to a tool whose policy is Require approval resolves — it does not throw — with `{ approval_required: true, approval_id }` instead of the tool's real result. Check for this shape with the SDK's `isApprovalRequired()` helper before treating a response as the real result; see \"Handling a require_approval response\" for the full recommended pattern.",
        "## Errors",
        "Both a bad request (wrong auth, unknown tool name) and a tool that ran but the underlying platform rejected it (e.g. a 404 from WooCommerce) throw the SDK's `HubApiError`, with `.message` and `.toolName` set — catch it once around a call rather than branching on HTTP status codes yourself.",
        "## A tool with no typed shortcut yet",
        "New canonical tools occasionally ship before an SDK release adds a typed `client.tools.*` entry for them. Call `client.call(\"the.tool.name\", { ...input })` directly in that case — it's exactly what every `client.tools.*` shortcut calls under the hood, so nothing is actually out of reach.",
        "Full reference, including every typed input shape this SDK version knows about, is in the package's own README on GitHub.",
      ],
      de: [
        "`@ai-integration/hub` verpackt die rohen JSON-RPC-Aufrufe hinter `client.call(name, input)` sowie einer typisierten Kurzform per Punktpfad — `client.tools.orders.refund({...})`, `client.tools.cms.pages.search({...})` — für jedes kanonische Tool. Er authentifiziert sich mit einem Projekt-API-Schlüssel und ist damit für einen serverseitigen Agenten oder ein Skript gedacht, nicht für eine Seite, die direkt im Browser eines Kunden lädt.",
        "## Installation",
        "Noch nicht im öffentlichen npm-Registry — installieren Sie ihn stattdessen aus seinem eigenen Repo: `npm install github:toasternet-gmbh/hub-sdk`. Der Build läuft automatisch beim Installieren, ein separater Klon-und-Build-Schritt entfällt. Eine Version fixieren statt dem Standard-Branch zu folgen geht mit `npm install github:toasternet-gmbh/hub-sdk#v0.1.0`.",
        "## Der erste Aufruf",
        "Einen Client pro Prozess anlegen: `new HubClient({ baseUrl, apiKey, agentId })`. `baseUrl` ist die MCP-Gateway-URL Ihres Hubs (endet auf `/functions/v1/hub-mcp-server`), `apiKey` stammt von der Seite API-Schlüssel, und `agentId` benennt, als welcher Agent dieser Prozess handelt — die meisten Tools benötigen sie, sie kann aber auch pro Aufruf statt einmalig gesetzt werden. Danach jedes Tool über seinen Punktpfad aufrufen, z. B. `client.tools.orders.search({ integration_id, status: 'pending_approval', limit: 10 })`.",
        "## Umgang mit require_approval",
        "Ein Aufruf eines Tools mit der Richtlinie Freigabe erforderlich löst kein Fehler aus — er liefert `{ approval_required: true, approval_id }` statt des echten Tool-Ergebnisses zurück. Prüfen Sie auf diese Form mit dem Helfer `isApprovalRequired()` des SDK, bevor Sie eine Antwort als echtes Ergebnis behandeln; das vollständige empfohlene Muster steht unter „Umgang mit einer require_approval-Antwort“.",
        "## Fehler",
        "Sowohl eine fehlerhafte Anfrage (falsche Authentifizierung, unbekannter Tool-Name) als auch ein Tool, das lief, dessen zugrunde liegende Plattform den Aufruf aber ablehnte (z. B. ein 404 von WooCommerce), lösen den `HubApiError` des SDK aus, mit gesetztem `.message` und `.toolName` — fangen Sie ihn einmal um einen Aufruf herum ab, statt selbst nach HTTP-Statuscodes zu verzweigen.",
        "## Ein Tool ohne typisierte Kurzform",
        "Gelegentlich erscheinen neue kanonische Tools, bevor eine SDK-Version einen typisierten `client.tools.*`-Eintrag dafür hat. Rufen Sie in diesem Fall direkt `client.call(\"das.tool.name\", { ...input })` auf — genau das ruft jede `client.tools.*`-Kurzform ohnehin im Hintergrund auf, nichts ist damit wirklich unerreichbar.",
        "Die vollständige Referenz, einschließlich jeder typisierten Eingabeform, die diese SDK-Version kennt, steht im README des Pakets auf GitHub.",
      ],
    },
  },
  {
