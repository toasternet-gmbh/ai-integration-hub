  ...
  },
  {
    ...
    body: {
      en: [
        ...
        "Call `tools/list` before writing any tool-specific code — it needs no API key or `agentId`, just a POST to your Hub's MCP gateway URL, and it's the exact catalog every `client.tools.*` shortcut is generated from. Each tool comes back with a `supported_platforms` array, so you always know which connected platform a given canonical tool actually works against, before you connect anything.",
        "```bash",
        "curl -X POST https://your-hub.example.com/functions/v1/hub-mcp-server \\",
        "  -H \"Content-Type: application/json\" \\",
        "  -d '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\",\"params\":{}}'",
        "```",
        "A tool in the response looks like this:",
        "```json",
        "{",
        "  \"name\": \"orders.refund\",",
        ...
        "}",
        "```",
        "By category, the canonical tool domains are:",
        ...
        "Which platforms actually support which tools varies within a category (not every e-commerce connector has `orders.refund`, for instance) — `tools/list`'s `supported_platforms` field is the source of truth, not this summary.",
      ...
      de: [
        ...
        "Rufen Sie `tools/list` auf, bevor Sie toolspezifischen Code schreiben — kein API-Schlüssel, keine `agentId` nötig, nur ein POST an die MCP-Gateway-URL Ihres Hubs, und genau der Katalog, aus dem jede `client.tools.*`-Kurzform generiert wird. Jedes Tool kommt mit einem `supported_platforms`-Array zurück, sodass Sie immer wissen, welche verbundene Plattform ein bestimmtes kanonisches Tool tatsächlich unterstützt — noch bevor Sie irgendetwas verbinden.",
        "```bash",
        "curl -X POST https://your-hub.example.com/functions/v1/hub-mcp-server \\",
        "  -H \"Content-Type: application/json\" \\",
        "  -d '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\",\"params\":{}}'",
        "```",
        "So sieht ein Tool in der Antwort aus:",
        "```json",
        "{",
        "  \"name\": \"orders.refund\",",
        "  \"description\": \"Refund an existing order (full or partial).\",",
        ...
        "}",
        "```",
        "Nach Kategorie sind die kanonischen Tool-Domains:",
        "- E-Commerce (WooCommerce, Shopware, Shopify, Magento, JTL, PrestaShop) — `orders.*`, `products.*`, `inventory.*`",
        ...
        "- Banking (GoCardless) — `accounts.*`, `transactions.*`",
        "- CRM (HubSpot) — `contacts.*`, `deals.*`",
        "Welche Plattform welche Tools tatsächlich unterstützt, variiert innerhalb einer Kategorie (nicht jeder E-Commerce-Connector hat z. B. `orders.refund`) — das Feld `supported_platforms` von `tools/list` ist die verbindliche Quelle, nicht diese Zusammenfassung.",
