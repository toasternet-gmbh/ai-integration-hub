...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  },
  {
    ...
    body: {
      en: [
        ...
        "- A freshly created integration has **no ACL role by default** — it will connect and pass \"Connect & test\", then fail on the first real call with \"Missing privilege\". Assign it the Administrator role, or at minimum `order:read`/`order:write` (for `orders.*`) and `product:read`/`product:write` (for `products.*` and `inventory.*` — Shopware doesn't split stock into its own ACL privilege), under the integration's Permissions tab.",
        ...
        "- Under Admin API access scopes, grant `read_orders`/`write_orders` (write is required for `orders.refund` — Shopify does not split refunds into a separate scope), plus `read_products`/`write_products` and `read_inventory`/`write_inventory` if you want `products.*`/`inventory.*` to work too (Shopify keeps inventory as a distinct scope pair from products).",
        ...
        "- Under API, grant Resource Access to at least Sales → Sales → Orders, and Sales → Operations → Credit Memos if you want `orders.refund` to work (a refund in Magento is created as a credit memo). Add Catalog → Products too if you want `products.*`/`inventory.*` to work — stock is part of the product resource in Magento, not a separate one.",
        ...
        "\"Connect & test\" immediately calls the platform to verify the credentials and discovers which canonical tools it supports — for a store, that's the `orders.*`, `products.*`, and `inventory.*` families (each needs its own scope/permission grant above; a credential missing one of them still connects fine and only fails when that specific family is actually called). A failure shows the platform's own error message — a wrong URL and a wrong secret produce different, specific errors, not a generic \"failed\". See \"Troubleshooting a failed store connection\" for what the common ones mean.",
      ...
      de: [
        ...
        "- Eine frisch erstellte Integration hat **standardmäßig keine ACL-Rolle** — sie verbindet sich und besteht „Verbinden & testen“, scheitert dann aber beim ersten echten Aufruf mit „Missing privilege“. Weisen Sie ihr im Reiter Berechtigungen die Administrator-Rolle zu, oder mindestens `order:read`/`order:write` (für `orders.*`) und `product:read`/`product:write` (für `products.*` und `inventory.*` — Shopware hat kein eigenes ACL-Recht für Lagerbestand).",
        ...
        "- Unter Admin-API-Zugriffsbereiche `read_orders`/`write_orders` gewähren (Schreiben ist für `orders.refund` erforderlich — Shopify hat für Rückerstattungen keinen eigenen Berechtigungsbereich), sowie `read_products`/`write_products` und `read_inventory`/`write_inventory`, falls `products.*`/`inventory.*` funktionieren sollen (Shopify führt Lagerbestand als eigenen Berechtigungsbereich getrennt von Produkten).",
        ...
        "- Unter API mindestens Ressourcenzugriff auf Sales → Sales → Orders gewähren, sowie Sales → Operations → Credit Memos, falls `orders.refund` funktionieren soll (eine Rückerstattung wird in Magento als Gutschrift/Credit Memo angelegt). Auch Catalog → Products hinzufügen, falls `products.*`/`inventory.*` funktionieren sollen — Lagerbestand ist in Magento Teil der Produktressource, keine eigene.",
        ...
        "## Verbindung prüfen",
        "„Verbinden & testen“ ruft die Plattform sofort auf, um die Zugangsdaten zu prüfen und die unterstützten kanonischen Tools zu ermitteln — bei einem Shop sind das die Familien `orders.*`, `products.*` und `inventory.*` (jede benötigt die oben genannte eigene Berechtigung; fehlt eine davon, verbindet sich die Integration trotzdem problemlos und scheitert erst, wenn genau diese Familie aufgerufen wird). Ein Fehlschlag zeigt die Originalfehlermeldung der Plattform — eine falsche URL und ein falsches Secret erzeugen unterschiedliche, konkrete Fehler statt eines generischen „fehlgeschlagen“. Was die häufigsten bedeuten, steht unter „Fehlerhafte Shop-Verbindung beheben“.",
  ...
  },
  {
    ...
    body: {
      en: [
        ...
        "Agents & Permissions → New agent, give it a name. It starts Active with the tool-registry defaults (see \"Setting tool permissions\"). Every call to a policy-gated canonical tool — `orders.*`, `invoices.*`, `cms.*`, and the rest of the domains that touch a connected platform — must carry an `X-Agent-Id` header identifying which agent is acting, and every one of those calls is written to the audit log. Setup/admin calls (connecting a platform, creating an agent, reviewing approvals, and the like) aren't gated the same way and don't need or check an agent id.",
      ...
      de: [
        ...
        "Agenten & Rechte → Neuer Agent, einen Namen vergeben. Er startet aktiv mit den Standardrechten aus dem Tool-Register (siehe „Tool-Rechte festlegen“). Jeder Aufruf eines richtliniengesteuerten kanonischen Tools — `orders.*`, `invoices.*`, `cms.*` und die übrigen Domänen, die eine verbundene Plattform berühren — muss einen `X-Agent-Id`-Header mit der handelnden Agent-ID enthalten, und jeder dieser Aufrufe wird im Audit-Log festgehalten. Einrichtungs-/Admin-Aufrufe (eine Plattform verbinden, einen Agenten anlegen, Freigaben prüfen und Ähnliches) sind nicht auf dieselbe Weise geprüft und benötigen keine Agent-ID.",
  ...
  },
  {
    ...
    body: {
      en: [
        ...
        "Every policy-gated canonical tool call — not just `orders.*`, every domain that touches a connected platform — also needs an `X-Agent-Id` header naming which agent is acting. This is separate from the API key/JWT, which only proves who or what is calling, not on whose behalf; setup/admin calls (creating an integration, an agent, and the like) don't need it.",
      ...
      de: [
        ...
        "Jeder Aufruf eines richtliniengesteuerten kanonischen Tools — nicht nur `orders.*`, jede Domäne, die eine verbundene Plattform berührt — benötigt außerdem einen `X-Agent-Id`-Header, der den handelnden Agenten benennt. Das ist unabhängig vom API-Schlüssel/JWT, der nur belegt, wer oder was aufruft, nicht in wessen Auftrag; Einrichtungs-/Admin-Aufrufe (eine Integration anlegen, einen Agenten anlegen und Ähnliches) benötigen ihn nicht.",
  ...
  },
  {
    ...
    category: "developer",
    readMins: 2,
    title: { en: "API keys and the JS/TS client", de: "API-Schlüssel und der JS/TS-Client" },
    summary: {
  ...
  },
  {
    ...
    body: {
      en: [
        "`@ai-integration/hub` wraps the raw JSON-RPC calls behind `client.call(name, input)` and a typed, dot-path shortcut — `client.tools.orders.refund({...})`, `client.tools.cms.pages.search({...})` — for every canonical tool. It authenticates with a project API key, so it's built for a server-side agent or script, not a page a customer's browser loads directly. Every example below is copy-pasteable as a starting point.",
        "## Install",
        "Not on the public npm registry yet — install it from its dedicated repo instead. It builds itself automatically on install, so there's no separate clone-and-build step.",
        "```bash",
        "npm install github:toasternet-gmbh/hub-sdk",
        "# or pin a release instead of tracking the default branch:",
        "npm install github:toasternet-gmbh/hub-sdk#v0.1.0",
        "```",
        "## Make your first call",
        "Construct one client per process, then call any tool by its dot-path. `baseUrl` is your Hub's MCP gateway URL (ends in `/functions/v1/hub-mcp-server`), `apiKey` comes from the API Keys page, and `agentId` names which agent this process acts as — most tools require it, though it can be overridden per call instead of set once.",
        "```ts",
        ...
        "Call `tools/list` before writing any tool-specific code — it needs no API key or `agentId`, and it's the exact catalog every `client.tools.*` shortcut is generated from. \"Discovering tools via the MCP protocol\" is the full reference (how to read a tool's `inputSchema`, what `supported_platforms` means, the raw JSON-RPC shape for a non-SDK client) — the short version:",
        "```bash",
        "curl -X POST https://your-hub.example.com/functions/v1/hub-mcp-server \\",
        ...
        "",
        "const result = await client.tools.orders.refund({",
        "  integration_id: \"int_...\",",
        "  order_id: \"ORD-8821A\",",
        "  reason: \"Customer reported item never arrived.\",",
        "});",
        "",
        ...
        "New canonical tools occasionally ship before an SDK release adds a typed `client.tools.*` entry for them. Call `client.call(...)` directly in that case — it's exactly what every `client.tools.*` shortcut calls under the hood, so nothing is actually out of reach.",
        "```ts",
      ...
      de: [
        "`@ai-integration/hub` verpackt die rohen JSON-RPC-Aufrufe hinter `client.call(name, input)` sowie einer typisierten Kurzform per Punktpfad — `client.tools.orders.refund({...})`, `client.tools.cms.pages.search({...})` — für jedes kanonische Tool. Er authentifiziert sich mit einem Projekt-API-Schlüssel und ist damit für einen serverseitigen Agenten oder ein Skript gedacht, nicht für eine Seite, die direkt im Browser eines Kunden lädt. Jedes Beispiel unten lässt sich direkt kopieren und als Ausgangspunkt verwenden.",
        "## Installation",
        "Noch nicht im öffentlichen npm-Registry — installieren Sie ihn stattdessen aus seinem eigenen Repo. Der Build läuft automatisch beim Installieren, ein separater Klon-und-Build-Schritt entfällt.",
        "```bash",
        "npm install github:toasternet-gmbh/hub-sdk",
        "# oder eine Version fixieren statt dem Standard-Branch zu folgen:",
        "npm install github:toasternet-gmbh/hub-sdk#v0.1.0",
        "```",
        "## Der erste Aufruf",
        "Einen Client pro Prozess anlegen und danach jedes Tool über seinen Punktpfad aufrufen. `baseUrl` ist die MCP-Gateway-URL Ihres Hubs (endet auf `/functions/v1/hub-mcp-server`), `apiKey` stammt von der Seite API-Schlüssel, und `agentId` benennt, als welcher Agent dieser Prozess handelt — die meisten Tools benötigen sie, sie kann aber auch pro Aufruf statt einmalig gesetzt werden.",
        "```ts",
        "import { HubClient } from \"@ai-integration/hub\";",
        "",
        "const client = new HubClient({",
        ...
        "Rufen Sie `tools/list` auf, bevor Sie toolspezifischen Code schreiben — kein API-Schlüssel, keine `agentId` nötig, und genau der Katalog, aus dem jede `client.tools.*`-Kurzform generiert wird. „Tools über das MCP-Protokoll entdecken“ ist die vollständige Referenz (wie man `inputSchema` liest, was `supported_platforms` bedeutet, die rohe JSON-RPC-Form für einen Client ohne SDK) — die Kurzfassung:",
        "```bash",
        "curl -X POST https://your-hub.example.com/functions/v1/hub-mcp-server \\",
        "  -H \"Content-Type: application/json\" \\",
        "  -d '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\",\"params\":{}}'",
        ...
        "  console.log(`Waiting on approval ${result.approval_id}`);",
        "} else {",
        "  // result ist das echte, bereits ausgeführte Ergebnis des Tools",
        "}",
        "```",
        "## Fehler",
        ...
        "Gelegentlich erscheinen neue kanonische Tools, bevor eine SDK-Version einen typisierten `client.tools.*`-Eintrag dafür hat. Rufen Sie in diesem Fall direkt `client.call(...)` auf — genau das ruft jede `client.tools.*`-Kurzform ohnehin im Hintergrund auf, nichts ist damit wirklich unerreichbar.",
        "```ts",
        "const result = await client.call(\"some.new.tool\", { integration_id: \"int_...\" });",
        "```",
  ...
  {
    slug: "discovering-tools-via-mcp",
    category: "developer",
    readMins: 6,
    title: { en: "Discovering tools via the MCP protocol", de: "Tools über das MCP-Protokoll entdecken" },
    summary: {
      en: "The raw JSON-RPC/MCP reference for finding out what's callable — for a custom MCP client or AI agent, not just the JS/TS SDK.",
      de: "Die rohe JSON-RPC-/MCP-Referenz, um herauszufinden, was aufrufbar ist — für einen eigenen MCP-Client oder KI-Agenten, nicht nur den JS/TS-SDK.",
    },
    body: {
      en: [
        "This article covers the raw MCP protocol underneath everything else — read it if you're building an AI agent that talks to the Hub directly (a custom MCP client, an agent framework with generic MCP support) rather than through the JS/TS SDK. Already using `@ai-integration/hub`? \"Using the SDK\" covers the same ground with less protocol detail.",
        "## Two ways to talk to the Hub",
        "Every canonical tool call — whatever client makes it — ends up as one JSON-RPC 2.0 POST to your Hub's MCP gateway URL (`.../functions/v1/hub-mcp-server`). The JS/TS SDK is a thin wrapper around exactly this protocol: `client.tools.orders.refund({...})` and `client.call(\"orders.refund\", {...})` both produce the same `tools/call` request described below. Nothing here requires the SDK.",
        "## The MCP handshake",
        "A standards-compliant MCP client calls three methods, in order: `initialize` (exchanges protocol version and capabilities), `tools/list` (what can I call?), and `tools/call` (do it). `initialize` and `tools/list` need no auth; `tools/call` does (see below).",
        "## tools/list is public",
        "`tools/list` needs no `Authorization` header and no API key — schema discovery (tool names, descriptions, input shapes, supported platforms) isn't customer data or credentials, it's the Hub's public API surface. Call it before you've even created an account.",
        "```bash",
        "curl -X POST https://your-hub.example.com/functions/v1/hub-mcp-server \\",
        "  -H \"Content-Type: application/json\" \\",
        "  -d '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\",\"params\":{}}'",
        ...
        "Every entry in the `tools` array `tools/list` returns has this shape — a real one, from the live server, not a placeholder:",
        "```json",
        "{",
        "  \"name\": \"orders.refund\",",
        ...
        "    }",
        "  },",
        "  \"supported_platforms\": [\"woocommerce\", \"shopware\", \"jtl\"]",
        "}",
        "```",
        "`inputSchema` is a standard JSON Schema object — `required` lists the fields you must supply, `properties` describes every field the tool accepts, including optional ones (`amount`, `reason` above). This is exactly what a well-behaved AI agent should parse to decide what arguments to pass, rather than guessing from the tool name or description alone.",
        "`supported_platforms` names which connected platforms actually implement this tool — check it against your integration's `platform` before calling, rather than finding out from a runtime error. Meta/admin tools that don't act on a specific connected platform (`create_integration`, `create_agent`, `list_approvals`, `create_organization`, ...) don't carry this field at all — they behave the same regardless of what's connected.",
        "## Canonical tool domains by category",
        "- E-commerce (WooCommerce, Shopware, Shopify, Magento, JTL, PrestaShop) — `orders.*`, `products.*`, `inventory.*`",
        "- Bookkeeping (Lexoffice, sevDesk, DATEV) — `invoices.*`, `contacts.*`",
        "- CMS (WordPress, TYPO3, Contentful) — `cms.pages.*`",
        "- Time tracking (Toggl Track, Personio, Clockify) — `time_entries.*`",
        "- Banking (GoCardless) — `accounts.*`, `transactions.*`",
        "- CRM (HubSpot) — `contacts.*`, `deals.*`",
        "Which platforms actually support which tools varies within a category (not every e-commerce connector has `orders.refund`, for instance) — the `supported_platforms` on the actual tool is the source of truth, not this summary.",
        "## Calling a tool once you've found it",
        "`tools/call` needs an `Authorization: Bearer` header — either a project-scoped API key (`hub_...`, from the API Keys page) or a signed-in user's Supabase JWT plus an explicit `project_id` in `arguments`. A canonical domain tool (any of the `*.` prefixes above) also needs an `X-Agent-Id` header naming which agent is acting, since every such call goes through the Policy Engine.",
        "```bash",
        "curl -X POST https://your-hub.example.com/functions/v1/hub-mcp-server \\",
        "  -H \"Content-Type: application/json\" \\",
        "  -H \"Authorization: Bearer hub_...\" \\",
        "  -H \"X-Agent-Id: <agent-uuid>\" \\",
        "  -d '{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/call\",\"params\":{\"name\":\"orders.search\",\"arguments\":{\"integration_id\":\"int_...\",\"limit\":10}}}'",
        "```",
        "The response comes back one of three ways: the tool's real result, `{ \"approval_required\": true, \"approval_id\": \"...\" }` if the tool's policy is Require approval (see \"Handling a require_approval response\"), or a JSON-RPC error. Using the JS/TS SDK instead of raw HTTP wraps all of this — auth headers, the require_approval shape, error handling — for you; see \"Using the SDK\".",
      ],
      de: [
        "Dieser Artikel behandelt das rohe MCP-Protokoll, das allem anderen zugrunde liegt — lesen Sie ihn, wenn Sie einen KI-Agenten bauen, der direkt mit dem Hub spricht (ein eigener MCP-Client, ein Agenten-Framework mit generischer MCP-Unterstützung) statt über den JS/TS-SDK. Nutzen Sie bereits `@ai-integration/hub`? „Der SDK im Einsatz“ deckt dasselbe ab, mit weniger Protokolldetails.",
        "## Zwei Wege, mit dem Hub zu sprechen",
        "Jeder kanonische Tool-Aufruf — egal welcher Client ihn macht — endet als ein JSON-RPC-2.0-POST an die MCP-Gateway-URL Ihres Hubs (`.../functions/v1/hub-mcp-server`). Der JS/TS-SDK ist nur eine dünne Hülle um genau dieses Protokoll: `client.tools.orders.refund({...})` und `client.call(\"orders.refund\", {...})` erzeugen beide dieselbe unten beschriebene `tools/call`-Anfrage. Nichts hier erfordert den SDK.",
        "## Der MCP-Handshake",
        "Ein standardkonformer MCP-Client ruft drei Methoden in dieser Reihenfolge auf: `initialize` (tauscht Protokollversion und Fähigkeiten aus), `tools/list` (was kann ich aufrufen?) und `tools/call` (tu es). `initialize` und `tools/list` benötigen keine Authentifizierung; `tools/call` schon (siehe unten).",
        "## tools/list ist öffentlich",
        "`tools/list` benötigt weder einen `Authorization`-Header noch einen API-Schlüssel — Schema-Discovery (Tool-Namen, Beschreibungen, Eingabeformen, unterstützte Plattformen) sind keine Kundendaten oder Zugangsdaten, sondern die öffentliche API-Oberfläche des Hubs. Rufen Sie es auf, bevor Sie überhaupt ein Konto angelegt haben.",
        "```bash",
        "curl -X POST https://your-hub.example.com/functions/v1/hub-mcp-server \\",
        "  -H \"Content-Type: application/json\" \\",
        "  -d '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\",\"params\":{}}'",
        "```",
        "## Eine Tool-Definition lesen",
        "Jeder Eintrag im `tools`-Array, das `tools/list` liefert, hat diese Form — eine echte, vom laufenden Server, kein Platzhalter:",
        "```json",
        "{",
        "  \"name\": \"orders.refund\",",
        "  \"description\": \"Refund an existing order (full or partial).\",",
        ...
        "}",
        "```",
        "`inputSchema` ist ein Standard-JSON-Schema-Objekt — `required` listet die Pflichtfelder auf, `properties` beschreibt jedes Feld, das das Tool akzeptiert, einschließlich optionaler Felder (hier `amount`, `reason`). Genau das sollte ein gut gebauter KI-Agent auswerten, um zu entscheiden, welche Argumente er übergibt, statt allein aus Name oder Beschreibung zu raten.",
        "`supported_platforms` nennt, welche verbundenen Plattformen dieses Tool tatsächlich implementieren — prüfen Sie es gegen die `platform` Ihrer Integration, bevor Sie aufrufen, statt es erst durch einen Laufzeitfehler zu erfahren. Meta-/Admin-Tools, die sich nicht auf eine bestimmte verbundene Plattform beziehen (`create_integration`, `create_agent`, `list_approvals`, `create_organization`, ...), tragen dieses Feld gar nicht — sie verhalten sich unabhängig davon, was verbunden ist.",
        "## Kanonische Tool-Domains nach Kategorie",
        "- E-Commerce (WooCommerce, Shopware, Shopify, Magento, JTL, PrestaShop) — `orders.*`, `products.*`, `inventory.*`",
        "- Buchhaltung (Lexoffice, sevDesk, DATEV) — `invoices.*`, `contacts.*`",
        "- CMS (WordPress, TYPO3, Contentful) — `cms.pages.*`",
        "- Zeiterfassung (Toggl Track, Personio, Clockify) — `time_entries.*`",
        "- Banking (GoCardless) — `accounts.*`, `transactions.*`",
        "- CRM (HubSpot) — `contacts.*`, `deals.*`",
        "Welche Plattform welche Tools tatsächlich unterstützt, variiert innerhalb einer Kategorie (nicht jeder E-Commerce-Connector hat z. B. `orders.refund`) — das `supported_platforms` am tatsächlichen Tool ist die verbindliche Quelle, nicht diese Zusammenfassung.",
        "## Ein gefundenes Tool aufrufen",
        "`tools/call` benötigt einen `Authorization: Bearer`-Header — entweder einen projektgebundenen API-Schlüssel (`hub_...`, von der Seite API-Schlüssel) oder das Supabase-JWT eines angemeldeten Nutzers plus eine explizite `project_id` in `arguments`. Ein kanonisches Domain-Tool (jedes der `*.`-Präfixe oben) benötigt zusätzlich einen `X-Agent-Id`-Header, der benennt, welcher Agent handelt, da jeder solche Aufruf durch die Policy Engine läuft.",
        "```bash",
        "curl -X POST https://your-hub.example.com/functions/v1/hub-mcp-server \\",
        "  -H \"Content-Type: application/json\" \\",
        "  -H \"Authorization: Bearer hub_...\" \\",
        ...
        "  -d '{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/call\",\"params\":{\"name\":\"orders.search\",\"arguments\":{\"integration_id\":\"int_...\",\"limit\":10}}}'",
        "```",
        "Die Antwort kommt auf eine von drei Arten zurück: das echte Ergebnis des Tools, `{ \"approval_required\": true, \"approval_id\": \"...\" }`, wenn die Richtlinie des Tools Freigabe erforderlich ist (siehe „Umgang mit einer require_approval-Antwort“), oder ein JSON-RPC-Fehler. Wer den JS/TS-SDK statt rohem HTTP nutzt, bekommt all das — Auth-Header, die require_approval-Form, Fehlerbehandlung — bereits eingepackt; siehe „Der SDK im Einsatz“.",
      ],
    },
  },
  {
  ...
  },
  {
    ...
    body: {
      en: [
        "Every call to a policy-gated canonical tool — `orders.*`, `products.*`, `inventory.*`, `invoices.*`, `contacts.*`, `cms.*`, `time_entries.*`, `accounts.*`, and `transactions.*` — is resolved in this order, most specific first: a rule for (this exact agent, this exact tool, this exact integration); then a rule for (this agent, this tool, all integrations); then the tool's own registry default. The first match wins — nothing is additive or averaged.",
      ...
      de: [
        "Jeder Aufruf eines richtliniengesteuerten kanonischen Tools — `orders.*`, `products.*`, `inventory.*`, `invoices.*`, `contacts.*`, `cms.*`, `time_entries.*`, `accounts.*` und `transactions.*` — wird in dieser Reihenfolge aufgelöst, spezifischste Regel zuerst: eine Regel für (genau dieser Agent, genau dieses Tool, genau diese Integration); dann eine Regel für (dieser Agent, dieses Tool, alle Integrationen); dann der Standardwert im Tool-Register selbst. Der erste Treffer gewinnt — nichts wird addiert oder gemittelt.",
  ...
  },
  {
    ...
    summary: {
      en: "The five steps from a fresh account to a safely-connected AI assistant.",
      de: "Die fünf Schritte von einem neuen Konto zu einem sicher verbundenen KI-Assistenten." },
    body: {
      en: [
        "1. Connect a platform — visit /connect (or click \"Connect a platform\" anywhere on the site), pick where your data lives, and fill in its connection details. This creates your account at the same time, so there's no separate sign-up first.",
        "2. Name your assistant — Agents & Permissions → New agent. Give it a name like \"Claude\" or \"Nightly automation\": this is the identity every action it takes gets traced back to.",
        "3. Get its key — API Keys → Create key. This is what you give your AI assistant — an MCP client like Claude Desktop, or your own code — so it can reach the Hub.",
        "4. That's already enough to read: with just that key, your assistant can look up orders, invoices, pages, whatever you connected. New agents start with safe defaults, so anything risky — like `orders.refund` — comes back `require_approval` instead of just running. That's correct, not a bug.",
        "5. Approve or deny from the Approvals inbox — the moment your assistant tries something that needs your OK, it shows up there, and nothing happens until you decide. From here, Audit Logs is where you go to see exactly what it did and when.",
      ],
      de: [
        "1. Plattform verbinden — besuchen Sie /connect (oder klicken Sie irgendwo auf der Seite auf „Plattform verbinden“), wählen Sie, wo Ihre Daten liegen, und tragen Sie die Zugangsdaten ein. Dabei wird gleichzeitig Ihr Konto angelegt — keine separate Registrierung nötig.",
        "2. Ihrem Assistenten einen Namen geben — Agenten & Rechte → Neuer Agent. Geben Sie ihm einen Namen wie „Claude“ oder „Nächtliche Automatisierung“: das ist die Identität, auf die sich jede seiner Aktionen zurückführen lässt.",
        "3. Seinen Schlüssel holen — API-Schlüssel → Schlüssel erstellen. Den geben Sie Ihrem KI-Assistenten — einem MCP-Client wie Claude Desktop, oder Ihrem eigenen Code —, damit er den Hub erreichen kann.",
        "4. Damit kann er schon lesen: Mit nur diesem Schlüssel kann Ihr Assistent Bestellungen, Rechnungen, Seiten nachschlagen — was auch immer Sie verbunden haben. Neue Agenten starten mit sicheren Standardwerten, sodass alles Riskante — wie `orders.refund` — mit `require_approval` zurückkommt, statt einfach ausgeführt zu werden. Das ist korrekt, kein Fehler.",
        "5. Im Freigabe-Postfach genehmigen oder ablehnen — sobald Ihr Assistent etwas versucht, das Ihre Freigabe braucht, landet es dort, und nichts passiert, bevor Sie entscheiden. Von hier aus sind die Audit-Protokolle die Anlaufstelle, um genau zu sehen, was er getan hat und wann.",
      ],
    },
  },
];
