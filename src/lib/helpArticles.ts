  ...
  },
  {
    ...
    body: {
      en: [
        ...
        "The Quick Start panel on that page shows a minimal working call using the official JS/TS client, `@ai-integration/hub` — copy it as a starting point for your own integration. It's not on the public npm registry yet; until it is, clone the Hub's repo and install the `sdk/` directory by local path (`npm install /path/to/ai-integration-hub/sdk`) — see that package's own README for exact steps. Full usage docs, including the `require_approval` handling pattern, are there too.",
      ],
      de: [
        ...
        "Das Schnellstart-Panel auf dieser Seite zeigt einen minimalen funktionierenden Aufruf mit dem offiziellen JS/TS-Client `@ai-integration/hub` — als Ausgangspunkt für Ihre eigene Integration kopierbar. Er ist noch nicht im öffentlichen npm-Registry; klonen Sie bis dahin das Repo des Hubs und installieren Sie das Verzeichnis `sdk/` per lokalem Pfad (`npm install /pfad/zu/ai-integration-hub/sdk`) — die genauen Schritte stehen im README dieses Pakets, ebenso die vollständige Nutzungsdokumentation.",
      ],
    },
  },
  {
