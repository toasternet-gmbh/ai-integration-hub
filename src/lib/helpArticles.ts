...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  },
  {
    ...
    summary: {
      en: "What credentials each supported platform needs, and how the Hub verifies the connection.",
      de: "Welche Zugangsdaten jede unterstützte Plattform benötigt und wie der Hub die Verbindung prüft.",
    },
    body: {
      en: [
        "Integrations → Connect a store. Pick a platform, name the connection (anything meaningful, e.g. \"Merch Store\"), and fill in its credential fields:",
        "WooCommerce: Store URL plus a Consumer Key/Secret pair from WooCommerce → Settings → Advanced → REST API. Shopware 6: Store URL plus a Client ID/Secret from an Integration created in Settings → System → Integrations. Shopify and Magento: Store URL plus a single Admin API access token (a custom app's token for Shopify, an Integration's Bearer token for Magento).",
        "\"Connect & test\" immediately calls the platform to verify the credentials and discovers which canonical tools it supports (`orders.search`/`orders.get`/`orders.refund` today). A failure shows the platform's own error message — a wrong URL and a wrong secret produce different, specific errors, not a generic \"failed\".",
        "Credentials are encrypted at rest and are never returned by any API response after the initial connect — not even to an admin. If you need to rotate a key, remove and re-add the integration.",
      ],
      de: [
        "Integrationen → Shop verbinden. Plattform wählen, der Verbindung einen aussagekräftigen Namen geben (z. B. „Merch Store“) und die Zugangsdaten eintragen:",
        "WooCommerce: Shop-URL plus Consumer Key/Secret aus WooCommerce → Einstellungen → Erweitert → REST-API. Shopware 6: Shop-URL plus Client-ID/Secret einer Integration aus Einstellungen → System → Integrationen. Shopify und Magento: Shop-URL plus ein einzelnes Admin-API-Zugriffstoken (Token einer Custom App bei Shopify, Bearer-Token einer Integration bei Magento).",
        "„Verbinden & testen“ ruft die Plattform sofort auf, um die Zugangsdaten zu prüfen und die unterstützten kanonischen Tools zu ermitteln (aktuell `orders.search`/`orders.get`/`orders.refund`). Ein Fehlschlag zeigt die Originalfehlermeldung der Plattform — eine falsche URL und ein falsches Secret erzeugen unterschiedliche, konkrete Fehler statt eines generischen „fehlgeschlagen“.",
        "Zugangsdaten werden verschlüsselt gespeichert und nach dem ersten Verbinden von keiner API-Antwort mehr zurückgegeben — auch nicht an einen Administrator. Zum Rotieren eines Schlüssels die Integration entfernen und neu anlegen.",
      ],
    },
  },
  {
